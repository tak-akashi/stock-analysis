...
def test_fixed_functions():
    ...
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) FROM relative_strength WHERE Date = ? AND RelativeStrengthIndex IS NOT NULL",
                (test_date,),
            )
            rsi_count = cursor.fetchone()[0]

            cursor.execute(
                "SELECT COUNT(*) FROM minervini WHERE Date LIKE ? AND Type_8 IS NOT NULL",
                (test_date + "%",),
            )
