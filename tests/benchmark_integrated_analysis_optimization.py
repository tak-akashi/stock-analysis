...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f"test_integrated_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
            ),
        ],
    )
...
def test_comprehensive_analysis():
    ...
    if not df.empty:
        ...
        if "composite_score" in df.columns:
            logger.info(
                f"Composite scores: min={df['composite_score'].min():.2f}, max={df['composite_score'].max():.2f}"
            )
        if "minervini_score" in df.columns:
            logger.info(
                f"Minervini scores: min={df['minervini_score'].min():.2f}, max={df['minervini_score'].max():.2f}"
            )
...
def main():
    ...
    try:
        ...
        if tests_passed == total_tests:
            logger.info(
                "🎉 ALL TESTS PASSED! Integrated analysis optimization is successful."
            )
            return True
        else:
