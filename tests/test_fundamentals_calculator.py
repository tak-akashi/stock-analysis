...
def calculator(temp_statements_db, temp_jquants_db, mock_cache):
    ...
    return FundamentalsCalculator(
        statements_db_path=temp_statements_db, jquants_db_path=temp_jquants_db
    )
...
class TestCalculationMethods:
    ...
    def test_calculate_per_normal(self):
        ...
        assert FundamentalsCalculator.calculate_per(2880, 180.5) == pytest.approx(
            15.96, rel=0.01
        )
    ...
    def test_calculate_pbr_normal(self):
        ...
        assert FundamentalsCalculator.calculate_pbr(2880, 2500) == pytest.approx(
            1.15, rel=0.01
        )
    ...
    def test_calculate_roe_normal(self):
        ...
        assert FundamentalsCalculator.calculate_roe(
            2500000000000, 35000000000000
        ) == pytest.approx(7.14, rel=0.01)
    ...
    def test_calculate_dividend_yield_normal(self):
        ...
        # Yield = DPS / Price * 100
        assert FundamentalsCalculator.calculate_dividend_yield(
            60, 2880
    ...
    def test_calculate_market_cap_normal(self):
        ...
        assert (
            FundamentalsCalculator.calculate_market_cap(2880, 14000000000)
            == 2880 * 14000000000
        )
    ...
    def test_calculate_operating_margin_normal(self):
        ...
        assert FundamentalsCalculator.calculate_operating_margin(
            3000000000000, 45000000000000
        ) == pytest.approx(6.67, rel=0.01)

    def test_calculate_profit_margin_normal(self):
        """Test profit margin calculation."""
        # Profit Margin = Profit / NetSales * 100
        assert FundamentalsCalculator.calculate_profit_margin(
            2500000000000, 45000000000000
        ) == pytest.approx(5.56, rel=0.01)

    def test_calculate_free_cash_flow_normal(self):
        """Test free cash flow calculation."""
        # FCF = Operating CF + Investing CF
        assert (
            FundamentalsCalculator.calculate_free_cash_flow(
                5000000000000, -2000000000000
            )
            == 3000000000000
        )
    ...
    def test_calculate_free_cash_flow_none(self):
        ...
        assert (
            FundamentalsCalculator.calculate_free_cash_flow(None, -2000000000000)
            is None
        )
        assert (
            FundamentalsCalculator.calculate_free_cash_flow(5000000000000, None) is None
        )
    ...
    def test_missing_price_data(self, temp_statements_db, mock_cache):
        ...
        try:
            calculator = FundamentalsCalculator(
                statements_db_path=temp_statements_db, jquants_db_path=empty_db_path
            )
            processed = calculator.update_all_fundamentals(temp_statements_db)
