...
class TestDatabaseFunctions:
    ...
    def test_save_stock_info_to_db(self):
        ...
        with patch(
            "market_pipeline.yfinance.data_processor.DB_PATH", self.test_db_path
        ):
    ...
    def test_save_stock_info_handles_missing_data(self):
        ...
        mock_info = {"symbol": "5678.T", "longName": "Minimal Company"}
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT ticker, longName, sector FROM stocks WHERE ticker = ?", ("5678.T",)
        )
        result = cursor.fetchone()
...
class TestDataFetching:
    ...
    @patch("market_pipeline.yfinance.data_processor.requests.get")
    def test_download_tse_listed_stocks_success(self, mock_get):
        ...
        with patch("market_pipeline.yfinance.data_processor.DATA_DIR", self.temp_dir):
    ...
    @patch("market_pipeline.yfinance.data_processor.download_tse_listed_stocks")
    @patch("market_pipeline.yfinance.data_processor.save_stock_info_to_db")
    @patch("market_pipeline.yfinance.data_processor.yf.Ticker")
    @patch("market_pipeline.yfinance.data_processor.pd.read_excel")
    def test_fetch_and_store_tse_data(
        self, mock_read_excel, mock_ticker, mock_save, mock_download
    ):
        ...
        with patch(
            "market_pipeline.yfinance.data_processor.time.sleep"
...
class TestTSEDataProcessor:
    ...
    @pytest.mark.skip(
        reason="Implementation uses pathlib.Path.mkdir instead of os.makedirs"
    )
    @patch("market_pipeline.yfinance.data_processor.init_db")
    @patch("os.makedirs")
    ...
    @pytest.mark.skip(
        reason="fetch_and_store_tse_data signature changed to include db_path"
    )
    @patch("market_pipeline.yfinance.data_processor.fetch_and_store_tse_data")
...
class TestIntegration:
    ...
    @pytest.mark.skip(
        reason="Integration test mocking is out of sync with current implementation"
    )
    @patch("market_pipeline.yfinance.data_processor.download_tse_listed_stocks")
    @patch("market_pipeline.yfinance.data_processor.yf.Ticker")
    @patch("market_pipeline.yfinance.data_processor.pd.read_excel")
