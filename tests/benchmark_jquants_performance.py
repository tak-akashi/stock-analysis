...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f"jquants_performance_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
            ),
        ],
    )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            for config in configurations:
                ...
                # Calculate metrics
                codes_per_second = (
                    len(test_codes) / elapsed_time if elapsed_time > 0 else 0
                )
                records_per_second = (
                    total_records / elapsed_time if elapsed_time > 0 else 0
                )
            ...
            for result in results:
                speedup_vs_best = (
                    result["time"] / best_config["time"]
                    if best_config["time"] > 0
                    else 1
                )
                ...
                if result["config"] == best_config["config"]:
                    ...
                    logger.info(
                        f"  {speedup_vs_best:.2f}x slower than {best_config['config']}"
                    )
                logger.info("")
    ...
    for batch_size in batch_sizes:
        ...
        try:
            ...
            processor = JQuantsDataProcessor(
                max_concurrent_requests=3, batch_size=batch_size, request_delay=0.1
            )
...
def test_error_recovery(test_codes: list):
    ...
    try:
        ...
        logger.info(
            f"  Recovery rate: {(successful / len(test_codes)) * 100:.1f}% (excluding invalid codes)"
        )

        return successful >= len(
            test_codes
        )  # Should successfully process all valid codes
...
def main():
    ...
    try:
        ...
        batch_results = test_batch_sizes(
            test_codes[:8]
        )  # Use fewer codes for batch test
        ...
        if performance_results:
            ...
            logger.info(
                f"   Processing rate: {best_config['codes_per_second']:.2f} codes/second"
            )
            logger.info(
                f"   Estimated time for 4000 codes: {(4000 / best_config['codes_per_second']) / 60:.1f} minutes"
            )
