...
| パス | 説明 |
|-----|------|
| `backend/config/settings.py` | Pydantic Settings による設定管理 |
| `backend/jquants/data_processor.py` | 非同期株価データ取得（~500行） |
| `backend/jquants/statements_processor.py` | 財務諸表取得（~400行） |
| `backend/jquants/fundamentals_calculator.py` | 財務指標計算（~300行） |
| `backend/analysis/minervini.py` | ミネルヴィニ分析 |
| `backend/analysis/high_low_ratio.py` | HL比率計算 |
| `backend/analysis/relative_strength.py` | RSP/RSI計算 |
| `backend/analysis/chart_classification.py` | チャートパターン分類 |
| `backend/analysis/integrated_analysis2.py` | Excel出力 |
| `backend/utils/parallel_processor.py` | 並列処理ラッパー |
| `backend/utils/cache_manager.py` | キャッシュ管理 |
