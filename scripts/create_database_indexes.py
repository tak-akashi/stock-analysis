...
def create_jquants_indexes(db_path: str):
    ...
    indexes = [
        {"name": "idx_daily_quotes_code", "table": "daily_quotes", "columns": ["Code"]},
        {"name": "idx_daily_quotes_date", "table": "daily_quotes", "columns": ["Date"]},
        {
    ...
    try:
        if filtered_indexes:
            ...
            logger.info(
                f"Successfully created {len(filtered_indexes)} indexes for jquants database"
            )
            if skipped_count > 0:
                logger.info(
                    f"Skipped {skipped_count} indexes due to existing constraints"
                )
        else:
...
def check_existing_constraints(db_path: str, table_name: str) -> Dict[str, List]:
    ...
    constraints = {"primary_keys": [], "unique_constraints": [], "existing_indexes": []}
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            for row in cursor.fetchall():
                ...
                constraints["existing_indexes"].append(
                    {"name": index_name, "columns": columns, "unique": bool(is_unique)}
                )
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            if original_count > dedup_count:
                ...
                cursor.execute(
                    f"INSERT INTO {table_name} SELECT * FROM {table_name}_dedup"
                )

                removed_count = original_count - dedup_count
                logger.info(
                    f"  Removed {removed_count} duplicate rows from {table_name}"
                )
...
def should_create_index(index_def: Dict, constraints: Dict) -> Tuple[bool, str]:
    ...
    for existing_idx in constraints["existing_indexes"]:
        if existing_idx["columns"] == index_columns:
            return (
                False,
                f"Index with same columns already exists: {existing_idx['name']}",
            )
...
def create_results_indexes(db_path: str):
                ...
                if should_create:
                    ...
                    if index_def.get("unique", False):
                        duplicate_count = check_duplicate_data(
                            db_path, table_name, index_def["columns"]
                        )
                        if duplicate_count > 0:
                            logger.warning(
                                f"  Found {duplicate_count} duplicate combinations for {index_def['name']}"
                            )
                            removed = clean_duplicate_data(
                                db_path, table_name, index_def["columns"]
                            )
                            if removed > 0:
                                logger.info(
                                    "  Cleaned duplicates, can now create unique index"
                                )
                            else:
                                logger.warning(
                                    "  Could not clean duplicates, skipping unique index"
                                )
                                skipped_count += 1
                                continue
    ...
    try:
        if filtered_indexes:
            db_processor.create_indexes(filtered_indexes)
            logger.info(
                f"Successfully created {len(filtered_indexes)} indexes for results database"
            )
            if skipped_count > 0:
                logger.info(
                    f"Skipped {skipped_count} indexes due to existing constraints"
...
def main():
    ...
    results_db_path = (
        "/Users/tak/Markets/Stocks/Stock-Analysis/data/analysis_results.db"
    )
    ...
    try:
        ...
        if not os.path.exists(results_db_path):
            logger.warning(
                f"Results database not found at {results_db_path}, will be created when needed"
            )
